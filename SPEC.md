...
命名規則ファイル（`BBB_attachment_*`）を別のページ（`AAA.md`）から参照している場合の動作:

**動作:**
- 添付ファイルは所有者ページ（`BBB.md`）にのみアップロードされる
- 参照元ページ（`AAA.md`）のリンクは、所有者ページの添付URL（`/attachment/{id}`）で置換される
...
**判定ルール:**
- ファイル名が `<ページ名>_attachment_*` パターンに一致
- かつ、現在処理中のページ名と `<ページ名>` が異なる場合
- → 外部参照としてマーク（アップロードせず、リンク置換のみ実行）

**通常ファイル（`image.png` など）:**
- 命名規則に一致しないファイルは、参照元ページそれぞれにアップロードされる（従来通り）
